...
//	private JButton btnMarkDone;
	...
	private void setWeekScope(int year, int month, int day) {
		...
		for (int i = 1; i < calWeekScope.length; i++) {
	...
	public void update(int year, int month, int day, Iterator<TestAppointment> activity) {
		
		this.setWeekScope(year, month, day);
		this.setTableNull();
		this.setTableValues(year, month, day, activity);
	}
	
	public void updateWeek(int year, int month, int day, Iterator<TestAppointment> activity) {
		
		this.setWeekScope(year, month, day);
		this.setTableNull();
		this.setTableValuesWeek(year, month, day, activity);
	}
	...
	public void setTableValues(int year, int month, int day, Iterator<TestAppointment> activity) {
		ArrayList<TestAppointment> listAppointment = new ArrayList<>();
		int rwCnt = 0;
		...
		if (activity == null) {
			...
			while (activity != null && activity.hasNext()) {
				listAppointment.add(activity.next());
			}
			
			for (TestAppointment t : listAppointment) {
				...
				if (isToday(t, year, month, day)) {
					...
					String time = t.getStartHour() + ":" + d.format(t.getStartMinute()) + " - " + t.getEndHour() + ":" + d.format(t.getEndMinute());
	...
	public void setTableValuesWeek(int year, int month, int day, Iterator<TestAppointment> activity) {
		ArrayList<TestAppointment> listAppointment = new ArrayList<>();
		int rwCnt = 0;
	...
	private boolean isThisWeek(TestAppointment t) {
		for(Calendar c : calWeekScope) {
			if(t.isMonth(c.get(Calendar.MONTH)) && t.isDay(c.get(Calendar.DAY_OF_WEEK)) && t.isYear(c.get(Calendar.YEAR))) {
				return true;
			}
		}
		return false;
	...
	/*
	public void update(int month, int day, int year, Iterator<Activity> activity) {
		this.setTableNull();
	...
	public void updateLabel(int year, int month, int day) {
		lblToDoLeft.setText("Unfinished task count: " + controller.getToDoLeft(year, month, day));
	}
	*//*
	public void deleteBtnDisable() {
		...
		flag = false;
		if (btnDeleteDone.getText().equalsIgnoreCase("Discard completed tasks")) {
			for (int i = 0; i < agendaTable.getRowCount(); i++) {
	...
	public void setTableValues(int month, int day, int year, Iterator<Activity> activity) {
		System.out.println("Setting table values");
		// ArrayList<Activity> listAppointment = new ArrayList<>();
		...
		if (activity == null) {
			...
			if (listAppointment.size() > 0)
				modelAgendaTable.setRowCount(listAppointment.size());
			for (int i = 0; i < listAppointment.size(); i++) {
				System.out.println("searching for act");
				if (listAppointment.get(i).isMonth(month) && listAppointment.get(i).isDay(day)
						&& listAppointment.get(i).isYear(year)) {
					System.out.println("add activity to agenda");
					String[] sArr = listAppointment.get(i).toStringArr();
	...
	private void addListeners() {
		...
		btnDeleteDone.addActionListener(new DeleteDone());
	}
	...
	class MarkDone implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (btnMarkDone.getText().equalsIgnoreCase("Mark tasks as completed...")) {
				btnMarkDone.setText("Mark as completed");
				btnDeleteDone.setText("Cancel");

				btnMarkDone.setEnabled(true);
				btnDeleteDone.setEnabled(true);

				setAgendaColumn(3);

				for (int i = 0; i < agendaTable.getRowCount(); i++) {
					if (agendaTable.getValueAt(i, 0) != null
							&& !((String[]) agendaTable.getValueAt(i, 0))[1].contains("-")
							&& ((String[]) agendaTable.getValueAt(i, 0))[1].contains(":")
							&& !((String[]) agendaTable.getValueAt(i, 1))[2].contains("lightgray")) {
						((AgendaTableModel) agendaTable.getModel()).setCellEditable(i, 2, true);

						String[] sArr = (String[]) agendaTable.getValueAt(i, 0);
						for (int j = 0; j < sArr.length; j++) {
							System.out.println("MARK " + sArr[j]);
						}

					} else {
						((AgendaTableModel) agendaTable.getModel()).setCellEditable(i, 2, false);
					}
				}
			} else if (btnMarkDone.getText().equalsIgnoreCase("Mark as completed")) {
				...
				for (int i = 0; i < agendaTable.getRowCount(); i++) {
					if (agendaTable.getValueAt(i, 2) != null && (boolean) agendaTable.getValueAt(i, 2)) {
						String[] sArr = (String[]) agendaTable.getValueAt(i, 0);
	...
	class DeleteDone implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			if (btnDeleteDone.getText().equalsIgnoreCase("Cancel")) {
				btnMarkDone.setText("Mark tasks as completed...");
				btnDeleteDone.setText("Discard completed tasks");

				setAgendaColumn(2);
			} else if (btnDeleteDone.getText().equalsIgnoreCase("Discard completed tasks")) {
